...
def main():
    ...
    file_name = f'{result_path}/evaluation_descriptive'
    ...
    if not os.path.isfile(file_path):
        ...
        for abstraction in ['F1', 'F2', 'F3', 'G1', 'G2', 'G3']:
            ...
            if abstraction[0] == 'F':
                ...
                mod = 'google'
            current_run = f'{mod}_{abstraction}_descriptive'
            if os.path.isfile(f'{result_path}/{current_run}.csv'):
                ...
                samples1 = [samples2_list[-1], samples2_list[0]]
                samples2 = [samples2_list[:-1], samples2_list[1:]]
                supports1 = [supp1, supp1]
                supports2 = [[supp2], [supp2]]
                up_mode_list = ['gen', 'spec']
                supp_sample = ['sample', 'sample']
        ...
        for supp2 in supps2:
            ...
            for matching in discovery:
                if matching == 'duc':
                    ...
                    start= time.time()
                    result_dict = update_query_set(result_dict, orig_sample, copy_sample, supp1, supp2,
...
def create_plots(dataframe:pd.DataFrame):
    ...
    grid.savefig('experiments/plots/gen_sample_descriptive.pdf')
    ...
    grid = sns.relplot(data=df_plot, x='# Deleted streams', y='Rel timechange', hue='Abstraction',
                       col='Dataset', kind='line', style='Abstraction', markers=True, legend=False)
    grid.set(yscale='log', yticks= [1,10,100], xticks=[5,10])
    grid.savefig('experiments/plots/spec_sample_descriptive.pdf')
    ...
    grid.savefig('experiments/plots/queryset_change_descriptive.pdf')
